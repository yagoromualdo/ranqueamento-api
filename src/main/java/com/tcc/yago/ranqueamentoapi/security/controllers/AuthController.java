...
import com.tcc.yago.ranqueamentoapi.security.payload.request.SignupRequestsTestes;
...
public class AuthController {
  ...
  @PostMapping("/signupTestes")
  public ResponseEntity<?> registerUsers(@Valid @RequestBody SignupRequestsTestes signupRequests) {
    for (SignupRequest signUpRequest : signupRequests.getRequests()) {
      if (userRepository.existsByUsername(signUpRequest.getUsername())) {
        return ResponseEntity.badRequest().body(new MessageResponse("Error: Username is already taken for " + signUpRequest.getUsername() + "!"));
      }

      if (userRepository.existsByEmail(signUpRequest.getEmail())) {
        return ResponseEntity.badRequest().body(new MessageResponse("Error: Email is already in use for " + signUpRequest.getEmail() + "!"));
      }
